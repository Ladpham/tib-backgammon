...
const BackgammonEngine = {
  ...
  isValidMove(G, player, from, to) {
    return true; // add real rules here
  },
  applyMove(G, player, from, to) {
    // mutate G.board.points, G.board.bar, G.board.off as needed
  },
  computeMetrics(history, rolls) {
    ...
    const sum = flatRolls.reduce((a,b)=>a+b, 0), n = flatRolls.length;
...
export const TIBGame = Game({
  name: 'tib-backgammon',
  setup: () => ({
    board: BackgammonEngine.initialBoard(),
    rolls: BackgammonEngine.generateDiceSeries(),
    history: [],
    currentRoll: null,
    matchTarget: 5,
  }),
  ...
  moves: {
    ...
    MoveChecker: {
      move: ({ G, ctx }, from, to) => {
        ...
        if (!BackgammonEngine.isValidMove(G, player, from, to)) return INVALID_MOVE;
        BackgammonEngine.applyMove(G, player, from, to);
  ...
  onEnd: ({ G, result }) => {
    const { pr, luck } = BackgammonEngine.computeMetrics(G.history, G.rolls);
